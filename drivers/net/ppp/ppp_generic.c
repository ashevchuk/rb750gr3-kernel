...
static void ppp_setup(struct net_device *dev)
{
	...
	dev->priv_flags |= IFF_FASTPATH | IFF_FASTSTATS;
}
...
ppp_xmit_process(struct ppp *ppp, struct sk_buff *skb)
{
	...
	ppp_xmit_lock(ppp);
			...
			if (!xcomp->lockless) xflags |= SC_COMP_RUN;
			comp = 1;
		}
	}

	if (xflags) ppp_xmit_lock(ppp);

	if (skb) {
		if (comp) {
			skb = pad_compress_skb(ppp, skb, !!xflags);
			if (!skb) {
				if (xflags) ppp_xmit_unlock(ppp);
				return;
			}
		}
		...
		if (!xflags && skb_queue_empty(&ppp->file.xq)) {
			if (!ppp_push(ppp, skb)) {
	...
	while ((skb = skb_dequeue(&ppp->file.xq))) {
		if (xflags & SC_MULTILINK ? !ppp_mp_explode(ppp, skb)
		    : !ppp_push(ppp, skb)) {
...
pad_compress_skb(struct ppp *ppp, struct sk_buff *skb, int locked)
{
	...
	if (xcomp->inplace &&
		...
		if (!locked && !xcomp->lockless) ppp_xmit_lock(ppp);
		len = xcomp->compress(xc_state, orgdata,
				      skb->data, orglen, skb->len);
		if (!locked && !xcomp->lockless) ppp_xmit_unlock(ppp);
